...
test('Episodes component renders correctly with episodes list', () => {
    ...
    //title text should be there
    //getByText because they are unique
    expect(getByText(/the lost sister/i)).toBeInTheDocument();
    expect(getByText(/Trick or Treat, Freak/i)).toBeInTheDocument();

    //synopsis text should be there
    expect(getByText(/The group splits up to close the gate, evict the Mind Flayer from Will, and drive the demo-dogs away from Hawkins Lab./i)).toBeInTheDocument();
    expect(getByText(/El travels to Chicago to find her sister Eight, aka Kali, who has teamed up with other outcasts to kill the men who tormented her at Hawkins Labs./i)).toBeInTheDocument();

    //minute text should be in ALL of them
    //getAllByText because there are multiple
    //toBeInDocument -> shows up in actual page
